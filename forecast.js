...
// by period. Short-period NW windswell that is still coupled to the wind lands
// in wind_wave and NEVER appears in the swell partitions -- which is why Chinese
// 10/12/25 read flat against an observed chest high (7ft @ 8s @ 306 deg was
// sitting in wind_wave the whole morning). Read as a fourth partition.
...
// ── Partition tracking ──────────────────────────────────────────────────────
// GFS reports only THREE swell partitions per hour and reshuffles which swells
// occupy those slots. A swell that is really there can vanish from the report
// for an hour and come back, which used to drop that hour's face height to 0
// and drag the 6-hour average down (Chinese 10/12/25 read 0.2ft against an
// observed chest high). Here we match partitions between hours by period and
// direction, then bridge short gaps: a swell reported before AND after a gap
// was physically present during it, so it is interpolated back in.
const TRACK_PER_TOL = 2.5;   // s   — same swell if period within this
const TRACK_DIR_TOL = 30;    // deg — and direction within this
const MAX_BRIDGE    = 2;     // hours of dropout we are willing to fill

function angDiff(a, b) { const d = Math.abs(a - b) % 360; return d > 180 ? 360 - d : d; }

function interpDir(a, b, f) {
  let d = ((b - a + 540) % 360) - 180;          // shortest signed arc
  return ((a + d * f) % 360 + 360) % 360;
}

// hourList: one partition array per hour → same shape with dropouts filled
function trackPartitions(hourList) {
  const tracks = [];
  hourList.forEach((parts, hi) => {
    parts.forEach(p => {
      let best = null, bestScore = Infinity;
      for (const t of tracks) {
        if (t.obs.has(hi)) continue;
        const dp = Math.abs(t.last.per - p.per), dd = angDiff(t.last.dir, p.dir);
        if (dp > TRACK_PER_TOL || dd > TRACK_DIR_TOL) continue;
        const score = dp / TRACK_PER_TOL + dd / TRACK_DIR_TOL + (hi - t.lastHour) * 0.5;
        if (score < bestScore) { bestScore = score; best = t; }
      }
      if (best) { best.obs.set(hi, p); best.last = p; best.lastHour = hi; }
      else tracks.push({ obs: new Map([[hi, p]]), last: p, lastHour: hi });
    });
  });
  const out = hourList.map(() => []);
  for (const t of tracks) {
    const hrs = [...t.obs.keys()].sort((a, b) => a - b);
    for (let i = 0; i < hrs.length - 1; i++) {
      const a = hrs[i], b = hrs[i + 1];
      if (b - a > 1 && b - a <= MAX_BRIDGE + 1) {
        const pa = t.obs.get(a), pb = t.obs.get(b);
        for (let h = a + 1; h < b; h++) {
          const f = (h - a) / (b - a);
          t.obs.set(h, { ht: pa.ht + (pb.ht - pa.ht) * f,
                         per: pa.per + (pb.per - pa.per) * f,
                         dir: interpDir(pa.dir, pb.dir, f), bridged: true });
        }
      }
    }
    for (const [h, p] of t.obs) out[h].push(p);
  }
  return out;
}

// ── Direction gate ──────────────────────────────────────────────────────────
// The reported partition direction is noisy — the same 14s swell was logged at
// 263° one hour and 304° the next. A hard cutoff turns that noise into a
// full-credit / zero-credit flip, so the window edges taper instead. This
// models uncertainty in the reported direction, NOT a claim that swell outside
// the window reaches the beach.
const EDGE_TAPER = 12;       // deg of taper beyond each end of swellDirs

function directionGate(dir, spotKey) {
  const [lo, hi] = SPOTS[spotKey].swellDirs;
  if (dir >= lo && dir <= hi) return 1;
  const out = dir < lo ? lo - dir : dir - hi;
  if (out >= EDGE_TAPER) return 0;
  return 0.5 * (1 + Math.cos(Math.PI * out / EDGE_TAPER));
}
...
function spotWave(d, spotKey) {
  ...
  const tracked = trackPartitions(SURF_HOURS.map(h => hours[h] || []));
  const hw = tracked.map(parts => hourWave(parts, spotKey));
  ...
  const card = hw[SURF_HOURS.indexOf(CARD_HOUR)] || hourWave(hours[CARD_HOUR] || [], spotKey);
